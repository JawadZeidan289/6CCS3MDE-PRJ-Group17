...
public class jsonlangTypingSystem extends XsemanticsRuntimeSystem {
  ...
  protected Result<JsonLangType> typeImpl(final RuleEnvironment G, final RuleApplicationTrace _trace_, final JsonProgram ex) throws RuleFailedException {
    try {
    	final RuleApplicationTrace _subtrace_ = newTrace(_trace_);
  ...
  protected Result<JsonLangType> applyRuleJSONLangObject(final RuleEnvironment G, final RuleApplicationTrace _trace_, final JsonProgram ex) throws RuleFailedException {
    
    return new Result<JsonLangType>(_applyRuleJSONLangObject_1(G, ex));
  }
  
  private JsonLangType _applyRuleJSONLangObject_1(final RuleEnvironment G, final JsonProgram ex) throws RuleFailedException {
    return JsonLangType.JSONPROGRAM;
  }
}
