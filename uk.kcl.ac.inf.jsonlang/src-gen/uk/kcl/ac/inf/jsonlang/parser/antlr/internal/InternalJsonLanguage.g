...
entryRuleJsonProgram returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getJsonProgramRule()); }
	iv_ruleJsonProgram=ruleJsonProgram
	{ $current=$iv_ruleJsonProgram.current; }
	EOF;

// Rule JsonProgram
ruleJsonProgram returns [EObject current=null]
@init {
	enterRule();
}
@after {
...
@after {
	...
	(
		...
		{
			$current = $this_JsonProgram_0.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
